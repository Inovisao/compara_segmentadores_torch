...
class SegmentationDataset(Dataset):
    ...
    def __getitem__(self, index):
        print("DEBUGGING: ", self.image_paths[index])
        image = cv2.imread(self.image_paths[index])
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB).astype('float32')
        image = image / 255.0

        mask_name = str("../data/masks/" +
                        self.image_paths[index][:-4].split("/")[-1] + ".png")
...
def print_data_informations(train_data, val_data, test_data, train_dataloader):
    ...
    print(
        f"Number of training images: {len(train_data)} ({100 * len(train_data) / total_images:>2f}%)")
    print(
        f"Number of validation images: {len(val_data)} ({100 * len(val_data) / total_images:>2f}%)")
    print(
        f"Number of test images: {len(test_data)} ({100 * len(test_data) / total_images:>2f}%)")
...
def get_data_loaders(train_dataset, test_dataset, val_split=DATA_HYPERPARAMETERS["VAL_SPLIT"], batch_size=DATA_HYPERPARAMETERS["BATCH_SIZE"]):
    train_idx, val_idx = train_test_split(
        list(range(len(train_dataset))), test_size=val_split)
    ...
    print_data_informations(train_dataset, val_dataset,
                            test_dataset, train_dataloader)
