...
# Get the flags
while getopts "k:s|S|h|H" flag; do
	case "${flag}" in
	k) ndobras=${OPTARG} ;;
	[sS])
		subset=1
...
echo "Checking if source directory exists: $dir_all"
if [ -d "$dir_all" ]; then
	echo "Source directory exists: $dir_all"
	echo "Listing contents of $dir_all:"
	ls -l $dir_all
else
	echo "Error: Source directory does not exist: $dir_all"
	exit 1
fi
...
if [[ $subset -eq 1 ]]; then
	for dir_class in $(ls $dir_train); do
		ids=()
		for img in $(ls ${dir_train}"/"${dir_class}); do
			string=$img"_"
...
if [[ $subset -eq 1 ]]; then
	for dir_fold in $(ls $dir_dobras); do
		for dir_class in $(ls ${dir_dobras}${dir_fold}); do
			for dir_item in $(ls ${dir_dobras}${dir_fold}"/"$dir_class); do
